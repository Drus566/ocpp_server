...
bool MessagesValidator::addValidator(const std::string& schemas_path, const std::string& action)
{
    ...
    std::experimental::filesystem::path req_filepath(schemas_path);
    req_filepath.append(action + ".json");
    ret = addValidator(req_filepath, action, true);

    // Add validator for response
    std::experimental::filesystem::path resp_filepath(schemas_path);
...
bool MessagesValidator::addValidator(const std::experimental::filesystem::path& validator_path, const std::string& action, bool is_req)
{
    bool ret = false;
