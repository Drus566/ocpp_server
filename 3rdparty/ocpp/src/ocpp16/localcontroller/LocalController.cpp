...
std::unique_ptr<ILocalController> ILocalController::create(const ocpp::config::ILocalControllerConfig& stack_config,
                                                           ILocalControllerEventsHandler&              events_handler)
{
    ...
    return std::unique_ptr<ILocalController>(new LocalController(stack_config, events_handler, timer_pool, worker_pool));
}
...
/** @brief Constructor */
LocalController::LocalController(const ocpp::config::ILocalControllerConfig&      stack_config,
                                 ILocalControllerEventsHandler&                   events_handler,
                                 std::shared_ptr<ocpp::helpers::ITimerPool>       timer_pool,
                                 std::shared_ptr<ocpp::helpers::WorkerThreadPool> worker_pool)
...
bool LocalController::resetData()
{
    bool ret = false;
...
void LocalController::rpcClientConnected(const std::string& chargepoint_id, std::shared_ptr<ocpp::rpc::RpcServer::Client> client)
{
...
void LocalController::processUptime()
{
    // Increase counters
    m_uptime++;
    m_total_uptime++;
    ...
    if ((m_uptime % 15u) == 0)
    {
        m_worker_pool->run<void>(std::bind(&LocalController::saveUptime, this));
    }
}
