...
PrivateKey::PrivateKey(const std::experimental::filesystem::path& pem_file, const std::string& passphrase)
...
bool PrivateKey::privateToFile(const std::experimental::filesystem::path& pem_file) const
{
    bool         ret = false;
...
bool PrivateKey::publicToFile(const std::experimental::filesystem::path& pem_file) const
{
    bool         ret = false;
    std::fstream x509_file(pem_file, std::fstream::out);
    if (x509_file.is_open())
