...
std::unique_ptr<ICentralSystem20> ICentralSystem20::create(const ocpp::config::ICentralSystemConfig20& stack_config,
                                                           ICentralSystemEventsHandler20&              events_handler)
{
    ...
    return std::unique_ptr<ICentralSystem20>(new CentralSystem20(stack_config, events_handler, timer_pool, worker_pool));
}
...
/** @brief Constructor */
CentralSystem20::CentralSystem20(const ocpp::config::ICentralSystemConfig20&      stack_config,
                                 ICentralSystemEventsHandler20&                   events_handler,
                                 std::shared_ptr<ocpp::helpers::ITimerPool>       timer_pool,
                                 std::shared_ptr<ocpp::helpers::WorkerThreadPool> worker_pool)
      ...
      m_total_uptime(0)
{
    ...
    if (m_timer_pool && m_worker_pool && m_internal_config)
    {
        ...
        m_uptime_timer->setCallback(std::bind(&CentralSystem20::processUptime, this));
    }
...
/** @brief Destructor */
CentralSystem20::~CentralSystem20()
{
    stop();
}

/** @copydoc bool ICentralSystem20::resetData() */
bool CentralSystem20::resetData()
{
    bool ret = false;
...
bool CentralSystem20::rpcAcceptConnection(const char* ip_address)
{
...
bool CentralSystem20::rpcCheckCredentials(const std::string& chargepoint_id, const std::string& user, const std::string& password)
{
    bool ret = false;
...
void CentralSystem20::rpcClientConnected(const std::string& chargepoint_id, std::shared_ptr<ocpp::rpc::RpcServer::Client> client)
{
    LOG_INFO << "Connection from Charge Point [" << chargepoint_id << "]";

    // Instanciate proxy
    std::shared_ptr<ICentralSystem20::IChargePoint20> chargepoint(
        new ChargePointProxy20(*this, chargepoint_id, client, m_messages_validator, m_messages_converter, m_stack_config));
...
void CentralSystem20::rpcClientFailedToConnect(const char* ip_address)
{
...
void CentralSystem20::rpcServerError()
{
    LOG_ERROR << "Critical server error";
}

/** @brief Initialize the database */
void CentralSystem20::initDatabase()
{
...
void CentralSystem20::processUptime()
{
    // Increase counters
    m_uptime++;
    m_total_uptime++;
    ...
    if ((m_uptime % 15u) == 0)
    {
        m_worker_pool->run<void>(std::bind(&CentralSystem20::saveUptime, this));
    }
}

/** @brief Save the uptime counter in database */
void CentralSystem20::saveUptime()
{
