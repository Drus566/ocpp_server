...
bool DeviceModelManager::init()
{
    ...
    std::experimental::filesystem::path device_model_schema_path = m_stack_config.jsonSchemasPath();
...
void DeviceModelManager::saveDeviceModel(rapidjson::Document& device_model_doc)
{
    ...
    // For each component
    for (const auto& components : m_device_model.components)
    {
        for (const auto& component : components.second)
        {
            ...
            for (const auto& vars : component.variables)
            {
                for (const auto& var_temp : vars.second)
                {
                    const auto& var = var_temp.second;
